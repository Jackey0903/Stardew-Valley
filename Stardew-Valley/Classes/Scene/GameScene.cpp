...
// 使用 GameManager::getInstance()->getCurrentMap() 替代全局变量
...
bool GameScene::init() {
  ...
  // 将地图置于屏幕中心
  tiledMap->setPosition(visibleSize.width / 2 + origin.x,
...
void GameScene::onEnter() {
  ...
  // [REFACTORED CODE - With Singleton Pattern]
  std::string selectedMap = GameManager::getInstance()->getCurrentMap();
  if (!selectedMap.empty() && selectedMap != currentMap) {
    CCLOG("GameScene: 切换到新地图: %s", selectedMap.c_str());
    ...
    if (mapLayer) {
      ...
      if (currentMap == "Map/Map1/map1.tmx") {
        ...
        // 计算地图的缩放比例，使其铺满窗口
        float scaleX =
            visibleSize.width / tiledMap->getContentSize().width * 1.5;
        float scaleY =
            visibleSize.height / tiledMap->getContentSize().height * 1.5;
        float scale =
            MAX(scaleX, scaleY); // 选择较大的缩放比例，确保地图铺满窗口
        tiledMap->setScale(scale);
        ...
        // 计算地图的缩放比例，使其铺满窗口
        float scaleX =
            visibleSize.width / tiledMap->getContentSize().width * 2.4;
        float scaleY =
            visibleSize.height / tiledMap->getContentSize().height * 2.4;
        float scale =
            MAX(scaleX, scaleY); // 选择较大的缩放比例，确保地图铺满窗口
        tiledMap->setScale(scale);
        ...
        // 计算地图的缩放比例，使其铺满窗口
        float scaleX =
            visibleSize.width / tiledMap->getContentSize().width * 0.8;
        float scaleY =
            visibleSize.height / tiledMap->getContentSize().height * 0.8;
        float scale =
            MAX(scaleX, scaleY); // 选择较大的缩放比例，确保地图铺满窗口
        tiledMap->setScale(scale);
        ...
        if (npcLayer) {
          ...
          for (const auto &obj : npcObjects) { // 将 Value 转换为 ValueMap
            auto npcObject = obj.asValueMap();
            // 检查是否为NPC对象
            if (npcObject["name"].asString() ==
                "Npc") // 假设你的NPC对象名称为"NPC1"
            {
              Vec2 npcPosition =
                  Vec2(npcObject["x"].asFloat(), npcObject["y"].asFloat());
    ...
    // [REFACTORED CODE - With Singleton Pattern]
    GameManager::getInstance()->clearCurrentMap();
  }
}
...
void GameScene::showDialogue() {
  auto dialogueBox =
      ui::Text::create("你好，勇士！", "fonts/Marker Felt.ttf", 24);
  dialogueBox->setPosition(
      Vec2(Director::getInstance()->getVisibleSize().width / 2,
           Director::getInstance()->getVisibleSize().height / 4));
  this->addChild(dialogueBox, 20);

  // 延迟3秒后移除对话框
  this->runAction(
      Sequence::create(DelayTime::create(3), RemoveSelf::create(), nullptr));
}
...
void GameScene::initEventListeners() {
  ...
  keyboardListener->onKeyReleased =
      CC_CALLBACK_2(GameScene::onKeyReleased, this);
...
void GameScene::plantCropAt(const Vec2 &locationInMap,
  ...
  // 输出日志，显示种植作物的位置
  CCLOG("Planted a crop (%s) at (%.2f,%.2f)", cropImages[cropIndex].c_str(),
        locationInMap.x, locationInMap.y);
...
void GameScene::harvestCropAt(const Vec2 &locationInMap) {
  ...
  for (auto it = _crops.begin(); it != _crops.end(); ++it) {
    ...
    if (boundingBox.containsPoint(locationInMap)) {
      ...
      _crops.erase(it);
      CCLOG("Harvested a crop at (%.2f,%.2f)", locationInMap.x,
            locationInMap.y);
      break;
    }
  }
...
void GameScene::onMouseDown(Event *event) {
  ...
  if (_isPlanting) {
    auto locationInView =
        Vec2(mouseEvent->getCursorX(), mouseEvent->getCursorY());
