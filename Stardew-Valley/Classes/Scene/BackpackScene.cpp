...
// 使用 GameManager::getInstance()->getPlayerSpeed() 替代 speed 全局变量
extern float size; // size 变量暂时保留，后续可迁移到 GameManager

// 创建并返回背包场景
Scene *BackpackScene::createScene() { return BackpackScene::create(); }
...
bool BackpackScene::init() {
  ...
  // 添加键盘事件监听器
  auto listener =
      EventListenerKeyboard::create(); // C++11特性：使用lambda函数作为回调
  listener->onKeyPressed =
      CC_CALLBACK_2(BackpackScene::onKeyPressed, this); // 按键按下事件
  _eventDispatcher->addEventListenerWithSceneGraphPriority(
      listener, this); // 为当前场景添加监听器
  ...
  auto backgroundSprite = Sprite::create(
      "Icons/background.png"); // C++11特性：使用智能指针（在Cocos2d中，资源管理通过智能指针）
  auto visibleSize =
      Director::getInstance()->getVisibleSize(); // 获取可见区域大小
  backgroundSprite->setPosition(
      Vec2(visibleSize.width / 2, visibleSize.height / 2)); // 设置背景位置
  // 计算并设置背景缩放比例
  ...
  // 创建玩家精灵并设置位置
  _playerSprite =
      Sprite::create("Action/Stand_Down.png"); // C++11特性：使用初始化列表
  _playerSprite->setPosition(
      Vec2(visibleSize.width * 0.75, visibleSize.height / 2)); // 设置精灵位置
  ...
  // 创建返回按钮
  _btnBack = cocos2d::ui::Button::create(
      "../Resources/KEYS/B.png"); // 确保有 "Icons/B.png" 资源
  _btnBack->setPosition(
      Vec2(KEYS_BACKPACK_X, KEYS_BACKPACK_Y)); // 根据需求调整位置
  _btnBack->setScale(0.2f);                    // 设置缩放比例
  _btnBack->setOpacity(opacity);               // 设置透明度
  this->addChild(_btnBack);

  // 设置返回按钮触摸事件
  _btnBack->addTouchEventListener(
...
void BackpackScene::createMenu() {
  ...
  for (int i = 0; i < items.size(); ++i) {
    ...
    auto menuItem = MenuItemImage::create(
        items[i]->getImagePath(), items[i]->getImagePath(),
...
void BackpackScene::createSkillTree() {
  ...
  // 创建移动速度滑动条标签
  auto speedLabel =
      Label::createWithTTF("移动速度", "fonts/Marker Felt.ttf", 20);
  speedLabel->setPosition(
      Vec2(origin.x + visibleSize.width - 150, origin.y + 100));
  ...
  _speedSlider->setPosition(
      Vec2(origin.x + visibleSize.width - 900 + BAR_OFFSET_X,
           origin.y + 300 + SPEED_BAR_OFFSET_Y));
  // [REFACTORED CODE - With Singleton Pattern]
  _speedSlider->setPercent(
      (GameManager::getInstance()->getPlayerSpeed() - 100) /
      2); // 设置默认百分比
  _speedSlider->addEventListener(
      CC_CALLBACK_2(BackpackScene::onSpeedSliderChanged, this)); // 事件监听
  this->addChild(_speedSlider, 40);

  // 创建人物大小滑动条标签
  auto sizeLabel =
      Label::createWithTTF("人物大小", "fonts/Marker Felt.ttf", 20);
  sizeLabel->setPosition(
      Vec2(origin.x + visibleSize.width - 300, origin.y + 100));
  ...
  _sizeSlider->setPosition(
      Vec2(origin.x + visibleSize.width - 900 + BAR_OFFSET_X,
           origin.y + 100 + SIZE_BAR_OFFSET_Y));
  _sizeSlider->setPercent((200.00 / 3.00) * size -
                          (100.00 / 3.00)); // 设置默认百分比（对应人物大小1.0）
  _sizeSlider->addEventListener(
      CC_CALLBACK_2(BackpackScene::onSizeSliderChanged, this)); // 事件监听
  this->addChild(_sizeSlider, 40);
}

// 移动速度滑动条变化时的回调
void BackpackScene::onSpeedSliderChanged(Ref *sender,
...
// 人物大小滑动条变化时的回调
void BackpackScene::onSizeSliderChanged(Ref *sender,
                                        ui::Slider::EventType type) {
  if (type == ui::Slider::EventType::ON_PERCENTAGE_CHANGED) {
...
void BackpackScene::updatePlayerWithItem() {
  if (_selectedItem) {
    // 更新玩家手上显示的物品图标
    _currentToolSprite->setTexture(
        _selectedItem->getImagePath()); // 更新物品图标
  }
}
